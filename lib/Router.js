...
class Router {
  ...
  [__MIDDLEWARE](ctx, next) {
    ...
    if (method === 'HEAD' && !node.methods.hasMiddleware('HEAD')) {
      method = 'GET'
    }
    ...
    if (method === 'OPTIONS') {
      ...
      ctx.response.set('Allow', node.methods.toString())
      return
    }

    // If there is no one middleware
    // it's a 405 error
    if (!node.methods.hasMiddleware(method)) {
      ctx.response.set('Allow', node.methods.toString())
    ...
    let fn = compose(node.methods.get(method))
    return fn(ctx, noop)
  }
}
